...
import { mat3, vec3 } from "./gl-matrix-min.js"
console.log("here");
...
function daltonizeRGB([red, green, blue], options) {
    ...
    vec3.transformMat3(rgb_simulated, lms, cvd);

    // 3. Calculate error matrix as the difference between the real RGB and simulated RGB with colorblindness
    const error = vec3.create();
    vec3.subtract(error, rgb, rgb_simulated);

    // 4. Shift colors towards visible spectrum (apply error modifications)
    const rgb_errorModification = vec3.create();
    vec3.transformMat3(rgb_errorModification, error, errorModificationMatrix)

    // 5. Add compensation to original RGB values
    const rgb_compensated = vec3.create();
    vec3.add(rgb_compensated, rgb_errorModification, rgb);

    // 6. Shift to convert any floating point values to integer quickly
    const result = [
        rgb_compensated[0] >> 0,
        rgb_compensated[1] >> 0,
        rgb_compensated[2] >> 0
    ...
    // 7. Clamp resulting RGB values to [0, 255]
    for (let i = 0; i < 3; i++) {
